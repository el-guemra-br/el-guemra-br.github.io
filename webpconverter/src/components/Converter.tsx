...
export default function Converter() {
  ...
  return (
    <section id="converter" className="py-16 bg-lavender-veil">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Quality Slider */}
        <div className="mb-6 bg-lavender-veil rounded-2xl p-5 border border-lavender">
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-semibold text-slate-700">
              Quality:{" "}
              <span className="text-lavender-grey font-bold">{quality}%</span>
            </label>
          ...
          <input
            ...
            className="w-full h-2 rounded-full accent-lavender-grey cursor-pointer"
          />
        ...
        <div
          ...
          className={`relative flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed cursor-pointer transition-all duration-200 py-16 px-8 text-center
            ${dragging
              ? "border-lavender-grey bg-lavender-veil scale-[1.01]"
              : "border-lavender bg-lavender-veil hover:border-lavender-grey hover:bg-lavender-veil"
            }`}
        >
          <input
            ref={fileInputRef}
          ...
          {converting ? (
            <div className="flex flex-col items-center gap-3">
              <div className="w-12 h-12 rounded-full border-4 border-lavender border-t-lavender-grey animate-spin" />
              <p className="text-slate-500 font-medium">Converting…</p>
            </div>
          ) : (
            <>
              <div className="w-16 h-16 rounded-2xl bg-linear-to-br from-lavender to-lavender-grey flex items-center justify-center shadow-lg shadow-lavender/20">
              ...
              <div>
                ...
                <p className="text-thistle text-sm mt-1">
                  or <span className="text-lavender-grey font-semibold underline underline-offset-2">browse files</span>
                </p>
              </div>
              <div className="flex flex-wrap justify-center gap-2">
                {["JPG", "PNG", "GIF", "BMP", "AVIF", "TIFF"].map((fmt) => (
                  <span key={fmt} className="px-2.5 py-1 rounded-full bg-lavender-veil border border-lavender text-thistle text-xs font-medium shadow-sm">
                    {fmt}
                  </span>
                ))}
              </div>
        ...
        {results.length > 0 && (
          <div className="mt-8 space-y-4">
            ...
            {results.map((file, idx) => (
              <div
                key={idx}
                className="flex items-center gap-4 p-4 rounded-2xl border border-lavender bg-lavender-veil hover:border-lavender-grey hover:bg-lavender-veil/60 transition-all"
              >
                {/* Preview */}
                <img
                  src={file.previewUrl}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-xs text-slate-500">
                    ...
                    <span>After: <b className="text-lavender-grey">{formatSize(file.webpSize)}</b></span>
                ...
                <a
                  ...
                  className="shrink-0 inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-lavender-grey hover:bg-lavender text-white text-sm font-semibold shadow transition-colors"
                >
